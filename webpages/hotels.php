...
<p>
  Choose from a selection of our hotel partners.
  <br>
  Costs are for hotel stays alone - please refer to our Packages for all inclusive offers.
</p>
...
$sql = "SELECT * FROM hotels" . ($where ? " WHERE " . implode(" AND ", $where) : "");

//Create prepared statements
$stmt = $pdo->prepare($sql);

//Attach given parameters to the statement 
$stmt->execute($param);

//Post statements into the HTML as a string so users can view their options
foreach ($stmt as $hotel) {
  echo 
  "<li><strong>{$hotel['name']}</strong>, {$hotel['company']}</li>
   <p>
    {$hotel['description']}
    <br>
    {$hotel['country']} 
    <br>
    <strong>Cost: </strong>£{$hotel['price']} 
    <br>
